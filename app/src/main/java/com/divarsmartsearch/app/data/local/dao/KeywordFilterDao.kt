...
interface KeywordFilterDao {
    ...
    // Built-in filters (دفتر/املاک/مشاور/کلید) can be turned off but never
    // deleted outright, so the WHERE clause only ever matches custom rows.
    @Query("DELETE FROM keyword_filters WHERE id = :id AND isBuiltIn = 0")
